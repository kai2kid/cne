...
<!--        <th width="250px">Entrance</th>-->
    ...
    <tbody>
      ...
      <tr>
          ...
          <td>
            <?php foreach([1,2,3] as $level) { ?>
              <?php echo text_mealLevel($level,0) . ": " . (isset($model->detail['restaurant'][$day][$level]) ? $model->detail['restaurant'][$day][$level]["menu_name"] . " - " . $model->detail['restaurant'][$day][$level]["restaurant_name"] : " - "); ?><br>
            <?php } ?>
          </td>
<!--          
          <td>
    ...
    <thead>
      <tr>
        ...
        <th class="text-right" width="110px">
          ...
          <input class="form-control num calc_save" type=number value="<?php echo $model->detail["calc"]["pax_custom"]; ?>" id="pax_custom" onchange="calculate(this.value)">
          <?php } ?>
...
//            $gtot[$pax]['*'] += $subt[$pax]; 
  ...
  <hr>
    ...
    <div class="form-group">
      <label for="rate_usd" class="control-label col-md-1 no-pad-r">Rate :</label>
      <div class="col-md-1 no-pad-r" style="margin-right: 5px;">
        <input type="number" id="rate_usd" class="form-control num calc_save" value = "<?php echo ($model->detail["calc"]["rate_usd"] > 0 ? $model->detail["calc"]["rate_usd"] : 0); ?>" onchange="calculate($('#pax_custom').val());">
      </div>
      <label for="pax_cnb" class="control-label col-md-1 no-pad-r">CNB :</label>
      <div class="col-md-1 no-pad-r" style="margin-right: 5px;">
        <input type="number" id="pax_cnb" class="form-control num calc_save" value = "<?php echo ($model->detail["calc"]["pax_cnb"] > 0 ? $model->detail["calc"]["pax_cnb"] : 0); ?>" onchange="calculate($('#pax_custom').val());">
      </div>
      <label for="pax_cbe" class="control-label col-md-1 no-pad-r">CEB :</label>
      <div class="col-md-1 no-pad-r" style="margin-right: 5px;">
        <input type="number" id="pax_ceb" class="form-control num calc_save" value = "<?php echo ($model->detail["calc"]["pax_ceb"] > 0 ? $model->detail["calc"]["pax_ceb"] : 0); ?>" onchange="calculate($('#pax_custom').val());">
      </div>
    </div>
  </div>        
  ...
  <table class="table table-striped table-bordered table-font" cellspacing="0" width="100%" id="table_master">
    ...
    <tbody>
          ...
          <td rowspan="6" width="250px">
            <table class="table-font" cellspacing="5px" cellpadding="5px" width="100%">
              <tr>
                ...
                <td><input type=number class="form-control num calc_save" id="maxmin_<?php echo $level; ?>" value="<?php echo $model->detail['calc']['maxmin_'.$level]; ?>"></td>
              </tr>
              <tr>
                <td><label class='control-label'>RANGE:</label></td>
                <td><input type=number class="form-control num calc_save" id="range_<?php echo $level; ?>" value="<?php echo $model->detail['calc']['range_'.$level]; ?>"></td>
              </tr>
              <tr>
                <td><label class='control-label'>SGL/SPPL:</label></td>
                <td><input type=number class="form-control num calc_save" id="sglspl_<?php echo $level; ?>" value="<?php echo $model->detail['calc']['sglspl_'.$level]; ?>"></td>
              </tr>
            </table>
          </td>
          <td>Grand Total</td>
      ...
      <tr>
          ...
          <?php ++$ctr; ?>
          <td align="right">
            <span title="<?php echo round(($gtot[$pax]['*'] + $gtot[$pax][$level]) / ($pax + 0.5*$model->detail["calc"]['pax_cnb'] + 0.75*$model->detail["calc"]['pax_ceb']) ); ?>" class="price_gt price_fare_krw pax_<?php echo $ctr; ?> level_<?php echo $level; ?>">
            <?php echo number_format(round(($gtot[$pax]['*'] + $gtot[$pax][$level]) / ($pax + 0.5*$model->detail["calc"]['pax_cnb'] + 0.75*$model->detail["calc"]['pax_ceb']))); ?>
            </span>
          </td>
          <?php }?>
      </tr>      
      ...
      <tr>
          ...
          <?php ++$ctr; ?>
          <?php 
                $decided[$pax][$level] = $fare[$pax][$level]['usd'] - $model->detail['calc']['maxmin_'.$level];
          ...
          <td align="right">
            <b>
              ...
              <input class="form-control num calc_save" type=number value="<?php echo ($model->detail['calc']['price_decided_'.$level.'_'.$ctr] == "" ? $decided[$pax][$level] : $model->detail['calc']['price_decided_'.$level.'_'.$ctr])  ?>" id="price_decided_<?php echo $level; ?>_<?php echo $ctr; ?>" onchange="changeDP(<?php echo $level; ?>,<?php echo $ctr; ?>)" class='price_gt price_decided pax_<?php echo $ctr; ?> level_<?php echo $level; ?>'>
          </td>
          <?php }?>
      </tr>      
      <tr>
